...
export default function BottomNav() {
  ...
  return (
    <div className="fixed bottom-4 left-4 right-4 z-40 flex justify-center">
      <nav className="relative flex items-center justify-around bg-slate-900 dark:bg-slate-950 rounded-[28px] px-4 py-2.5 shadow-2xl shadow-black/30 w-full max-w-md">
        {tabs.map((tab) => {
          const active = tab.path === '/trips'
          ...
          if (tab.isCenter) {
            return (
              ...
              >
                ...
                {/* White ring */}
                <div className="absolute inset-0 -m-1 rounded-full border-4 border-slate-900 dark:border-slate-950 pointer-events-none" />
              </button>
            );
          }
          ...
          return (
            <button
              key={tab.path}
              onClick={() => navigate(tab.path === '/trips' ? '/' : tab.path)}
